package com.kkkoke.lottery.domain.activity.service.partake.impl;
...
import com.kkkoke.lottery.domain.activity.service.partake.BaseActivityPartake;
    ...
    public Result recordDrawOrder(DrawOrderVO drawOrder) {
        try {
            ...
            return transactionTemplate.execute(status -> {
                try {
                    // 锁定活动领取记录
                    int lockCount = userTakeActivityRepository.lockTackActivity(drawOrder.getuId(), drawOrder.getActivityId(),
                            drawOrder.getTakeId());
                    if (lockCount == 0) {
                        status.setRollbackOnly();
