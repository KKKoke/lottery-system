package com.kkkoke.lottery.interfaces.facade;
...
import com.kkkoke.lottery.interfaces.assembler.IMapping;
...
public class LotteryActivityBooth implements ILotteryActivityBooth {
    ...
    public DrawRes doDraw(DrawReq drawReq) {
        try {
            ...
            // 1. 执行抽奖
            DrawProcessResult drawProcessResult = activityProcess.doDrawProcess(new DrawProcessReq(drawReq.getuId(),
                    drawReq.getActivityId()));
    ...
    public DrawRes doQuantificationDraw(QuantificationDrawReq quantificationDrawReq) {
        try {
            ...
            RuleQuantificationCrowdResult ruleQuantificationCrowdResult =
                    activityProcess.doRuleQuantificationCrowd(new DecisionMatterReq(quantificationDrawReq.getTreeId(),
                            quantificationDrawReq.getuId(), quantificationDrawReq.getValMap()));
            ...
            DrawProcessResult drawProcessResult =
                    activityProcess.doDrawProcess(new DrawProcessReq(quantificationDrawReq.getuId(), activityId));
            if (!Constants.ResponseCode.SUCCESS.getCode().equals(drawProcessResult.getCode())) {
